function updateRobotVideo(theme) {
  const lightVideo = document.querySelector(".hero-robot-light");
  const darkVideo = document.querySelector(".hero-robot-dark");

  if (!lightVideo || !darkVideo) {
    return;
  }

  // Pausa os dois vídeos
  lightVideo.pause();
  darkVideo.pause();
  ...
  if (theme === "dark") {
    ...
    const playPromise = darkVideo.play();
...
(function () {
  ...
  function setupTheme() {
    ...
    function applyTheme() {
      ...
      // CORREÇÃO: Chama a função para atualizar o vídeo do robô ao mudar o tema
      if (typeof updateRobotVideo === "function") {
        updateRobotVideo(resolved);
      }
